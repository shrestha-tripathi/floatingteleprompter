...
const DEFAULT_DOMAIN = "floatingteleprompter.com";
...
export const site = {
  /** Brand / product name. */
  name: env.PUBLIC_SITE_NAME ?? "Floating Teleprompter",
  /** Short brand name for tight UI (nav, footer wordmark, OG site_name). */
  shortName: env.PUBLIC_SITE_SHORT_NAME ?? "Teleprompter",
  /**
   * Short tagline — feeds the homepage <title> (composed as "Brand — tagline")
   * and og:image:alt. Kept short so the homepage title stays ≤ 60 chars and
   * doesn't truncate in Google SERPs.
   */
  tagline:
    env.PUBLIC_SITE_TAGLINE ?? "A teleprompter that floats over any app",
  /** One-sentence description for <meta description> + OG. */
  description:
    env.PUBLIC_SITE_DESCRIPTION ??
    "Free online teleprompter that floats over OBS, Zoom, Loom and any recording app. Paste your script, hit play, look at the camera. No install, no signup — runs entirely in your browser.",
  ...
  /** Maker credit for the About/Contact pages (E-E-A-T trust signal). */
  ...
  githubRepo:
    ...
    "https://github.com/shrestha-tripathi/floatingteleprompter",
 ...
 * `absoluteUrl("/faq")` -> "https://floatingteleprompter.com/faq/". The single
